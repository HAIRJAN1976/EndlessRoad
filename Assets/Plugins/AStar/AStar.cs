...
public struct Node
{
    ...
    public Node(Vector2 position, bool isBlock = false) : this()
    {
        to = position;
        block = false;
...
public class AStar : MonoBehaviour
{
    ...
    public void Initialize(int size, Vector2 endPosition)
    {
        fieldSize = size;
    ...
    // A*探索アルゴリズムを開始する
    public IEnumerator Search(Vector2 positionStart)
    {
        

        // スタート地点の初期化
        openNodes[(int)positionStart.x, (int)positionStart.y] = new Node(new Vector2(0, 0))
        {
            from = positionStart,
            moveCost = 0,
            empty = false
    ...
    IEnumerator DrawLine()
    {
        ...
        //myLine.transform.position = positionStart;
        //myLine.AddComponent<LineRenderer>();
        //LineRenderer lr = myLine.GetComponent<LineRenderer>();

        //string path = "etc/line";
        //Material material = Resources.Load<Material>(path);

        //lr.material = material;
        //lr.SetColors(new Color(192, 221, 255), new Color(59, 115, 185));
        //lr.SetWidth(0.01f, 0.01f);
        //lr.SetPosition(0, positionStart);
        //lr.SetPosition(1, positionEnd);
